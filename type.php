...
<body>
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse navbar-ex1-collapse">
					<ul class="nav navbar-nav">
						<li class=""><a href="http://localhost/mlgia/" class="menu">Home</a></li>
					<?php foreach ($rs['product_type'] as $type): ?>
						<li class="<?php if ($type['type_id']==$id): ?>
							active
						<?php endif ?>"><a href="?action=type$<?php echo $type['type_id']   ?>" class="menu"><?php echo $type['type_name']  ?></a></li>
					<?php endforeach ?>
					</ul>
				</div><!-- /.navbar-collapse -->
			</div>
	...
	<div class="content">
		<div class="container">
			<div class="row">
				<a class="feature line-bottom" href="#"><?php echo  $name_type  ?></a>

				<?php 
					if (isset($rs['product_typeRS'])) {
						$numRow=count( $rs['product_typeRS'])/4;
						for ($i=0; $i <$numRow ; $i++) { 
							?>
							<div class="row">
								<?php 
								...
								$end=$start+4;
								for ($j=$start; $j <$end ; $j++) { 
									if (isset($rs['product_typeRS'][$j])) {
										?>
										<div class="col-md-3">
					<div class="items">
						<img src="public/images/<?php  echo $rs['product_typeRS'][$j]['img1']   ?>">
						<a href="#" class="pname"><?php  echo $rs['product_typeRS'][$j]['name']  ?></a>
						<p class="pprice"><?php  echo $rs['product_typeRS'][$j]['price']  ?>$</p>
					</div>	
				</div>
